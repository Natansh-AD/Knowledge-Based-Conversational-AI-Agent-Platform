...
const AgentsPage = () => {
  ...
  // Top bar button → navigate to create page
  useEffect(() => {
    if (setTopBarActions) {
      setTopBarActions(
        <button
              ...
              agents.map((agent) => (
                <tr key={agent.id}>
                    ...
                    {agent.tags_detail?.map((tag) => (
                      <span
                        ...
                        className={`px-2 py-1 text-xs rounded cursor-pointer
                          ${filters.tag.includes(tag.id) ? "bg-gray-900 text-white" : "bg-gray-200 hover:bg-gray-300"}`}
                      >
                        {tag.name}
                      </span>
                    ))}
                  ...
                  <td className="px-4 py-2 space-x-2">
                    <button onClick={() => navigate(`/${org}/agents/${agent.id}/edit`)} className="text-sm text-blue-600 hover:underline">Edit</button>
                    <button onClick={() => handleDelete(agent.id)} className="text-sm text-red-600 hover:underline">Delete</button>
                    <button onClick={() => handleStartChat(agent.id)} className="text-sm text-gray-800 hover:underline">Chat</button>
                  </td>
                </tr>
              ))
            )}
